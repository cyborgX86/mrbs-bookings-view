...
sistema.*/
 	...
 	while($row = mysqli_fetch_array($qry)) { //$row debe estar definida aquí.
		...
		if ( ($dateBookingEnd > $date) || ($timeBookingEnd >= $time) ){
 			echo '<li><div class="box"><table><tr>
 						<th class="pop" width="25%">Día ' . substr($row[3],8,2). '-' . substr($row[3],5,2).
 						' de ' . substr($row[2],11,5) . ' a '  . substr($row[3],11,5) . '</th>
						<th></th></tr>
						<tr><th colspan="2" class="booking"> ' . utf8_encode ($row['name']) . '</th></tr>
						<tr><th class="pop">' . utf8_encode ($row['room_name']);
 ...
 function getIndicators($qry){
	 ...
	 echo '<div><ul id="indicators" class="indicators"><li class="active"><em></em></li>';
